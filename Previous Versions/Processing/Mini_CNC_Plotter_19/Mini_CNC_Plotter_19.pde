...
//   -- Mini CNC Plotter - 19 ---
//
//   Code has been modified a lot since the last version (18). I didn't save
//   many mods becasue they were not stable. This is a stable version that
//   does what exaclty I want (waiting for reply for an indefinite period)
//
//   Author : Vishnu M Aiea
//   E-Mail : vishnumaiea@gmail.com
//   Web : www.vishnumaiea.in
//   Date created : 12:20 PM 27-04-2017, Thursday
//   Last modified : 03:45:40 PM, 26-05-2017, Friday
...
void plotImage () {
    ...
    while ((isCurrentPortActive()) && (!ifStopPlotter) && (!isPlottingFinished)) { //plotting loop
      while((!ifPausePlotter) && (!ifStopPlotter) && (!isPlottingFinished) && (isCurrentPortActive())) {
        ...
        if ((!ifPlotterReady) && (isCurrentPortActive()) && (!ifStopPlotter)) { //if plotter is not ready yet
          ...
          if (serialReceiveRespone().equals("READY!")) { //read response
          ...
          if ((ifLines) && (!isPlottingFinished) && (isCurrentPortActive()) && (!ifStopPlotter)) {
            if (currentSegment > lineCount) { //check if finished plotting all lines
              ...
              if(waitingForResponse) {
                //if (serialReceiveRespone().equals("DONE!")) {
                if (serialCheckRespone("DONE!")) {
                  waitingForResponse = false;
                  //serialPort.clear(); //get ready to receive another
                }
              }
            }
          }
  ...
  if(ifStopPlotter && isPlotterActive) {
    println("Plotting has been terminated !");
    isPlotterActive = false;
  }
...
boolean serialCheckRespone (String expectedResponse) {
  if(isCurrentPortActive()) {
    if (serialPort.available() > 0) {
      ...
      if(receivedResponse != null) {
        receivedResponse = removeDelimiter(receivedResponse);
        int responseLength = receivedResponse.length();
        receivedResponse = receivedResponse.replace("\n", "<CR>");
        println("Received Response = " + receivedResponse + " (" + responseLength + ")");
        println("Expected Response = " + expectedResponse + " (" + expectedResponse.length() + ")");
        if (receivedResponse.contains(expectedResponse)) {
          println("Checked Response = " + receivedResponse);
          println();
          waitingForResponse = false;
          serialPort.clear();
...
boolean serialSendCommand(String commandString) {
  if(isCurrentPortActive()) { //check if port is alive
    ...
    serialPort.clear();
    println("Send command : " + commandString);
...
String getEndOfLine (int a) {
  if((a > -1) && (a <= lineCount)) { //index should be between 0-lineCount
    String tempString = "X";
