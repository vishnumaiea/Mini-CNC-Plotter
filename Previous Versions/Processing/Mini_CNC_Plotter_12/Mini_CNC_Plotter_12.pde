...
//   -- Mini CNC Plotter - 12 --
//
//   Fixed some bugs in image selection prompt and UI buttons. The UI 
//   button problem (isClcked() didn't work as expected when there was
//   multiple buttons) was fixed by assigning a mouseStatus flasg to each
//   button, instead of a global one.
//
//   Author : Vishnu M Aiea
//   E-Mail : vishnumaiea@gmail.com
//   Web : www.vishnumaiea.in
//   Date created : 12:20 PM 27-04-2017, Thursday
//   Last modified : 12:00 PM 04-05-2017, Thursday
...
void setup() {
  ...
  fontIcon startIcon = new fontIcon (283, 466, "", fontAwesome, 16, colorMediumBlack, colorBlue, colorOrange);
  fontIcon pauseIcon = new fontIcon (283, 511, "", fontAwesome, 16, colorMediumBlack, colorBlue, colorOrange);
  fontIcon resumeIcon = new fontIcon (283, 511, "", fontAwesome, 16, colorMediumBlack, colorBlue, colorOrange);
  fontIcon stopIcon = new fontIcon (283, 556, "", fontAwesome, 16, colorMediumBlack, colorBlue, colorOrange);
  ...
  plotterStartIcon = startIcon;
  plotterPauseIcon = pauseIcon;
  plotterResumeIcon = resumeIcon;
  plotterStopIcon = stopIcon;
}
...
void showMainWindow() {
  ...
  if ((startMainWindow) && (!isPortError) && (isPortActive(activePortName))) {
    ...
    if(isImageLoaded) {
      image(imageName, (imageBoxX+5), (imageBoxY+5));
    }
    else {
    ...
    if((loadImageButton.isClicked()) && (!isFilePromptOpen)) {
      if(!isImageLoaded) {
        isFilePromptOpen = true;
        selectInput("Select a PNG file :", "selectImageFile");
      }
    }
    
    if((isImageSelected) && (!isImageLoaded)) {
      imageName = loadImage(imageFilePath, "png");
